...
async function resolveDependency(specifier, parent, job, cjsResolve = true) {
    ...
    if (cacheItem) {
        await Promise.all(cacheItem.filesToEmit.map(async (item) => {
    ...
    if (path_1.isAbsolute(specifier) || specifier === '.' || specifier === '..' || specifier.startsWith('./') || specifier.startsWith('../')) {
        ...
        resolved = await resolvePath(path_1.resolve(parent, '..', specifier) + (trailingSlash ? '/' : ''), parent, job, filesToEmit);
    }
    else if (specifier[0] === '#') {
...
;
async function resolvePath(path, parent, job, filesToEmit) {
...
async function resolveFile(path, parent, job, filesToEmit) {
    if (path.endsWith('/'))
        return undefined;
    path = await job.realpath(path, parent, undefined, filesToEmit);
    if (await job.isFile(path))
        return path;
    if (job.ts && path.startsWith(job.base) && path.substr(job.base.length).indexOf(path_1.sep + 'node_modules' + path_1.sep) === -1 && await job.isFile(path + '.ts'))
        return path + '.ts';
    if (job.ts && path.startsWith(job.base) && path.substr(job.base.length).indexOf(path_1.sep + 'node_modules' + path_1.sep) === -1 && await job.isFile(path + '.tsx'))
        return path + '.tsx';
...
async function resolveDir(path, parent, job, filesToEmit) {
    if (path.endsWith('/'))
        path = path.slice(0, -1);
    ...
    if (pkgCfg && typeof pkgCfg.main === 'string') {
        const resolved = await resolveFile(path_1.resolve(path, pkgCfg.main), parent, job, filesToEmit) || await resolveFile(path_1.resolve(path, pkgCfg.main, 'index'), parent, job, filesToEmit);
        if (resolved) {
            filesToEmit.push({ file: path + path_1.sep + 'package.json', parent, type: 'resolve' });
            return resolved;
        }
    }
    return resolveFile(path_1.resolve(path, 'index'), parent, job, filesToEmit);
}
...
async function packageImportsResolve(name, parent, job, cjsResolve, filesToEmit) {
        ...
        if (pjsonBoundary) {
            ...
            if (pkgCfg && pkgImports !== null && pkgImports !== undefined) {
                ...
                if (importsResolved) {
                    ...
                    if (importsResolved) {
                        filesToEmit.push({ file: pjsonBoundary + path_1.sep + 'package.json', parent, type: 'resolve' });
...
async function resolvePackage(name, parent, job, cjsResolve, filesToEmit) {
    ...
    while ((separatorIndex = packageParent.lastIndexOf(path_1.sep)) > rootSeparatorIndex) {
        ...
        if (job.conditions && pkgExports !== undefined && pkgExports !== null && !selfResolved) {
            let legacyResolved;
            if (!job.exportsOnly)
                legacyResolved = await resolveFile(nodeModulesDir + path_1.sep + name, parent, job, filesToEmit) || await resolveDir(nodeModulesDir + path_1.sep + name, parent, job, filesToEmit);
            ...
            if (resolved) {
                filesToEmit.push({ file: nodeModulesDir + path_1.sep + pkgName + path_1.sep + 'package.json', parent, type: 'resolve' });
            ...
            const resolved = await resolveFile(nodeModulesDir + path_1.sep + name, parent, job, filesToEmit) || await resolveDir(nodeModulesDir + path_1.sep + name, parent, job, filesToEmit);
            if (resolved) {
