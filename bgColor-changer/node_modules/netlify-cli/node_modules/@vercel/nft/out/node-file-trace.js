...
const fsReadlink = util_1.promisify(fs_1.default.readlink);
...
class Job {
    ...
    async realpath(path, parent, seen = new Set(), filesToEmit) {
        if (seen.has(path))
    ...
    async emitFile(path, reason, parent, isRealpath = false) {
        if (!isRealpath)
        ...
        await Promise.all([
            ...[...assets].map(async (asset) => {
                ...
                if (ext === '.js' || ext === '.mjs' || ext === '.node' || ext === '' ||
                    this.ts && (ext === '.ts' || ext === '.tsx') && asset.startsWith(this.base) && asset.substr(this.base.length).indexOf(path_1.sep + 'node_modules' + path_1.sep) === -1)
